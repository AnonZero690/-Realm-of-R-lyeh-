...
namespace RealmOne.Rarities
{
    ...
    {
        public override Color RarityColor => new Color(83, 57, 140);

        public override int GetPrefixedRarity(int offset, float valueMult)
        {
            if (offset > 0)
            { // If the offset is 1 or 2 (a positive modifier).
                return ModContent.RarityType<InkyColo>(); // Make the rarity of items that have this rarity with a positive modifier the higher tier one.
            }
