...
//#
    ...
    public class RealmModPlayer: ModPlayer
    {
        ...
        {
            if (brassy && Player.controlUp && Player.releaseUp)
            {
                Player.AddBuff(BuffID.Ironskin, 300);
                SoundEngine.PlaySound(SoundID.Item20, Player.position);
                for (int i = 0; i < 10; i++)
                {
                    int num = Dust.NewDust(Player.position, Player.width, Player.height, DustID.CopperCoin, 0f, -2f, 0, default, 2f);
                    Main.dust[num].noGravity = true;
                    Main.dust[num].position.X += Main.rand.Next(-50, 51) * .05f - 1.3f;
                    Main.dust[num].position.Y += Main.rand.Next(-50, 51) * .05f - 1.3f;
       ...
       public override void OnHitNPCWithItem(Item item, NPC target, NPC.HitInfo hit, int damageDone)/* tModPorter If you don't need the Item, consider using OnHitNPC instead */
        {
          if (Overseer && Main.rand.NextBool(2) && !target.friendly && hit.Crit  && target.lifeMax > 10 && target.type != NPCID.TargetDummy)
            {
        ...
        {
            if (Overseer && Main.rand.NextBool(2) && hit.Crit && !target.friendly && target.lifeMax > 10 && !target.SpawnedFromStatue && target.type != NPCID.TargetDummy)
           {
                Player.AddBuff(ModContent.BuffType<OverseerBuff>(), 400);
           }
       }
