...
namespace RealmOne.Global
{
    public class GlobalItemList : GlobalItem
    {
        ...
        {
            if (item.type == ItemID.Wood) item.ammo = item.type;
            if (item.type == ItemID.MudBlock) item.ammo = item.type;
            if (item.type == ItemID.Cactus) item.ammo = item.type;
            ...
            if (item.type == ItemID.Blowpipe) item.damage = 16;
            if (item.type == ItemID.Blowpipe) item.shootSpeed = 27f;


            if (item.type == ItemID.Blowgun) item.damage = 48;
            if (item.type == ItemID.Blowgun) item.shootSpeed = 30f;
            ...
            if (item.type == ItemID.EnchantedBoomerang) item.damage = 20;
            if (item.type == ItemID.EnchantedBoomerang) item.useTime = 18;
            if (item.type == ItemID.EnchantedBoomerang) item.shootSpeed = 14;

            if (item.type == ItemID.WoodenBoomerang) item.damage = 13;
            if (item.type == ItemID.WoodenBoomerang) item.useTime = 18;
            if (item.type == ItemID.WoodenBoomerang) item.shootSpeed = 11;

            

            if (item.type == ItemID.IceBoomerang) item.damage = 22;
            if (item.type == ItemID.IceBoomerang) item.useTime = 18;    
            if (item.type == ItemID.IceBoomerang) item.shootSpeed = 14;

            if (item.type == ItemID.Flamarang) item.damage = 49;
            if (item.type == ItemID.Flamarang) item.useTime = 18;
            if (item.type == ItemID.Flamarang) item.shootSpeed = 18;
            ...
            //PUMPKIN MOON
            if (item.type == ItemID.TheHorsemansBlade) item.damage = 120;
            if (item.type == ItemID.TheHorsemansBlade) item.useTime = 17;
            if (item.type == ItemID.TheHorsemansBlade) item.useAnimation = 17;

            if (item.type == ItemID.TheHorsemansBlade) item.crit = 10;
            if (item.type == ItemID.TheHorsemansBlade) item.knockBack = 6;
            if (item.type == ItemID.TheHorsemansBlade) item.scale = 1.5f;

            if (item.type == ItemID.RavenStaff) item.damage = 80;
            if (item.type == ItemID.RavenStaff) item.useTime = 16;
            if (item.type == ItemID.RavenStaff) item.crit = 12;
            if (item.type == ItemID.RavenStaff) item.knockBack = 4;
            if (item.type == ItemID.RavenStaff) item.autoReuse = true;
            if (item.type == ItemID.RavenStaff) item.shootSpeed = 20f;

            if (item.type == ItemID.BatScepter) item.damage = 78;
            if (item.type == ItemID.BatScepter) item.useTime = 9;
            if (item.type == ItemID.BatScepter) item.crit = 8;
            if (item.type == ItemID.BatScepter) item.knockBack = 6;
            if (item.type == ItemID.BatScepter) item.shootSpeed = 23f;

            if (item.type == ItemID.CandyCornRifle) item.damage = 70;
            if (item.type == ItemID.CandyCornRifle) item.useTime = 8;
            if (item.type == ItemID.CandyCornRifle) item.crit = 6;
            if (item.type == ItemID.CandyCornRifle) item.knockBack = 6;

            if (item.type == ItemID.JackOLanternLauncher) item.damage = 100;
            if (item.type == ItemID.JackOLanternLauncher) item.useTime = 20;
            if (item.type == ItemID.JackOLanternLauncher) item.crit = 8;
            if (item.type == ItemID.JackOLanternLauncher) item.knockBack = 10;
            if (item.type == ItemID.JackOLanternLauncher) item.shootSpeed = 22f;

            if (item.type == ItemID.ScytheWhip) item.damage = 100;
            if (item.type == ItemID.ScytheWhip) item.crit = 8;
            if (item.type == ItemID.ScytheWhip) item.knockBack = 6;
            if (item.type == ItemID.ScytheWhip) item.shootSpeed = 18f;


            if (item.type == ItemID.StakeLauncher) item.damage = 86;
            if (item.type == ItemID.StakeLauncher) item.useTime = 13;
            if (item.type == ItemID.StakeLauncher) item.crit = 20;
            if (item.type == ItemID.StakeLauncher) item.knockBack = 4;
            if (item.type == ItemID.StakeLauncher) item.shootSpeed = 27f;
            ...
            if (item.type == ItemID.SDMG) item.damage =  84;
            ...
            //Demolitionist Changes
            if (item.type == ItemID.Grenade) item.damage = 35;

            if (item.type == ItemID.Grenade) item.DamageType = ModContent.GetInstance<DemolitionClass>();


            if (item.type == ItemID.StickyGrenade) item.damage = 35;

            if (item.type == ItemID.StickyGrenade) item.DamageType = ModContent.GetInstance<DemolitionClass>();


            if (item.type == ItemID.Beenade) item.DamageType = ModContent.GetInstance<DemolitionClass>();

            if (item.type == ItemID.PartyGirlGrenade) item.damage = 32;

            if (item.type == ItemID.PartyGirlGrenade) item.DamageType = ModContent.GetInstance<DemolitionClass>();

            if (item.type == ItemID.BouncyGrenade) item.damage = 35;

            if (item.type == ItemID.BouncyGrenade) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            ...
            if (item.type == ItemID.Bomb)  item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.Bomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.StickyBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.StickyBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.Dynamite) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.Dynamite) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.StickyDynamite) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.StickyDynamite) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.BombFish) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.BombFish) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.BouncyBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.BouncyBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.BouncyDynamite) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.BouncyDynamite) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.DirtBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.DirtBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.DirtStickyBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.DirtStickyBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.WetBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.WetBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.DryBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.DryBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.ScarabBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.ScarabBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.HoneyBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.HoneyBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.LavaBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.LavaBomb) item.DamageType = ModContent.GetInstance<DemolitionClass>();



            if (item.type == ItemID.GrenadeLauncher) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.ProximityMineLauncher) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.RocketLauncher) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.JackOLanternLauncher) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.SnowmanCannon) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.Stynger) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.ElectrosphereLauncher) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.FireworksLauncher) item.DamageType = ModContent.GetInstance<DemolitionClass>();
            if (item.type == ItemID.Celeb2) item.DamageType = ModContent.GetInstance<DemolitionClass>();


            //Hooks
            if (item.type == ItemID.GrapplingHook) item.damage = 14;
            if (item.type == ItemID.AmethystHook) item.damage = 17;
            if (item.type == ItemID.SquirrelHook) item.damage = 15;
...

        {
            ...
            Recipe blow1 = Recipe.Create(ItemID.Blowgun);
            blow1.AddIngredient(Mod,"WoodenGunBarrel", 1);
            blow1.AddIngredient(ItemID.Acorn, 5);
            blow1.AddTile(TileID.WorkBenches);
            blow1.Register();


            Recipe BAND = Recipe.Create(ItemID.BandofRegeneration);
            BAND.AddIngredient(ItemID.LesserHealingPotion, 5);
            BAND.AddRecipeGroup("IronBar", 5);

            BAND.AddTile(TileID.Anvils);
            BAND.Register();

            Recipe MBAND = Recipe.Create(ItemID.BandofStarpower);
            MBAND.AddIngredient(ItemID.ManaCrystal, 2);
            MBAND.AddRecipeGroup("IronBar", 5);

            MBAND.AddTile(TileID.Anvils);
            MBAND.Register();

            Recipe fireflower = Recipe.Create(ItemID.FlowerofFire);
            fireflower.AddIngredient(ItemID.HellstoneBar, 12);
            fireflower.AddIngredient(ItemID.Fireblossom, 5);

            fireflower.AddIngredient(ItemID.Bone, 10);

            fireflower.AddTile(TileID.Hellforge);
            fireflower.Register();

            Recipe darklance = Recipe.Create(ItemID.DarkLance);
            darklance.AddIngredient(ItemID.HellstoneBar, 8);
            darklance.AddIngredient(ItemID.DemoniteBar, 15);

            darklance.AddIngredient(ItemID.Bone, 10);

            darklance.AddTile(TileID.Hellforge);
            darklance.Register();

            Recipe darklance1 = Recipe.Create(ItemID.DarkLance);
            darklance1.AddIngredient(ItemID.HellstoneBar, 8);
            darklance1.AddIngredient(ItemID.CrimtaneBar, 15);

            darklance1.AddIngredient(ItemID.Bone, 10);

            darklance1.AddTile(TileID.Hellforge);
            darklance1.Register();


            Recipe marrow = Recipe.Create(ItemID.Marrow);
            marrow.AddIngredient(Mod,"MossMarrow", 1);
            marrow.AddIngredient(ItemID.SoulofNight, 8);

            marrow.AddIngredient(ItemID.Bone, 10);

            marrow.AddTile(TileID.Hellforge);
            marrow.Register();
            ...
            Recipe BeeGun = Recipe.Create(ItemID.BeeGun);
            BeeGun.AddIngredient(ItemID.BeeWax, 12);
            BeeGun.AddIngredient(ItemID.Hive, 8);
            BeeGun.AddIngredient(ItemID.ManaCrystal, 1);
            BeeGun.AddTile(TileID.Anvils);
            BeeGun.Register();


            Recipe aglet = Recipe.Create(ItemID.Aglet);
            aglet.AddIngredient(ItemID.Cobweb, 5);
            aglet.AddRecipeGroup("IronBar", 2);
            aglet.AddTile(TileID.WorkBenches);
            aglet.Register();

          

            Recipe guide = Recipe.Create(ItemID.GuideVoodooDoll);
            guide.AddIngredient(ItemID.Leather, 5);
            guide.AddIngredient(Mod,"Flamestone", 5);

            guide.AddTile(TileID.Loom);
            guide.Register();
            ...
            climbingclaws1.AddIngredient(ItemID.TinBar, 5);
            ...
            piggybank.AddIngredient(Mod, "PiggyPorcelain", 35);
            piggybank.AddTile(TileID.Anvils);
            piggybank.Register();

            Recipe piggybank2 = Recipe.Create(ItemID.MoneyTrough);
            piggybank2.AddIngredient(Mod, "PiggyPorcelain", 50);
            piggybank2.AddTile(TileID.Anvils);
            piggybank2.Register();

            Recipe bugnet = Recipe.Create(ItemID.BugNet);
            bugnet.AddIngredient(ItemID.Cobweb, 15);
            bugnet.AddIngredient(Mod, "BrassIngot", 3);
            bugnet.AddTile(TileID.Loom);
            bugnet.Register();


            Recipe sharp = Recipe.Create(ItemID.SharpeningStation);
            sharp.AddIngredient(ItemID.Wood, 18);
            sharp.AddRecipeGroup("IronBar", 15);
            sharp.AddTile(TileID.WorkBenches);
            sharp.Register();
        }
