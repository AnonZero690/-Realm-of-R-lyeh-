...
namespace RealmOne.Projectiles.Other
{
    internal class CactiSticky : ModProjectile
    {
        ...
        private void NormalAI()
        {
            ...
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.height, Projectile.width, DustID.t_Cactus, Projectile.velocity.X * .2f, Projectile.velocity.Y * .2f, 200, Scale: 1.2f);
                dust.velocity += Projectile.velocity * 0.3f;
                dust.velocity *= 0.2f;
            }
            if (Main.rand.NextBool(4))
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.height, Projectile.width, DustID.t_Cactus, 0, 0, 254, Scale: 0.3f);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                // Create a new dust
                Dust dust = Dust.NewDustDirect(usePos, Projectile.width, Projectile.height, DustID.OasisCactus);
